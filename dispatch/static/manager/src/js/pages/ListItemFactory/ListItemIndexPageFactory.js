...
function ListItemIndexPageFactory(typeString) {
  return class ListItemsPageComponent extends React.Component {
    ...
    render() {
      ...
      const titleString = typeString.replace(/^\w/, m => m.toUpperCase())
      ...
      return (
        <DocumentTitle title={titleString}>
          <ItemList
            ...
            columns={[
              item => (<strong><Link to={`/${typeString}/${item.id}`} dangerouslySetInnerHTML={{__html: item.name}} /></strong>),
              item => item.slug
            ]}

            emptyMessage={`You haven\'t created any ${typeString} yet.`}
            createHandler={() => (
              <LinkButton intent={Intent.SUCCESS} to={`${typeString}/new`}>
                <span className='pt-icon-standard pt-icon-add'></span>Create {typeString}
              </LinkButton>)
            }
...
export default ListItemIndexPageFactory
