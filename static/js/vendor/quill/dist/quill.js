...
var Delta, Document, Embed, Format, Line, LinkedList, Normalizer, dom, _;
...
Document = (function() {
  function Document(root, editor, options) {
    this.root = root;
    this.editor = editor;
    if (options == null) {
      options = {};
  ...
  Document.prototype.appendEmbed = function(embedNode, type, data) {
  ...
  Document.prototype.insertEmbedBefore = function(newEmbedNode, refLine, type, data) {
    var embed;
    embed = new Embed(this, newEmbedNode, type, data);
    if (refLine != null) {
      if (!dom(newEmbedNode.parentNode).isElement()) {
        this.root.insertBefore(newEmbedNode, refLine.node);
      }
      this.lines.insertAfter(refLine.prev, embed);
    } else {
      if (!dom(newEmbedNode.parentNode).isElement()) {
        this.root.appendChild(newEmbedNode);
      }
      this.lines.append(embed);
    }
    this.lineMap[embed.id] = embed;
    return embed;
  };
  ...
  Document.prototype.buildFromJSON = function(data) {
    this.lines = new LinkedList();
    this.lineMap = {};
    this.root.innerHTML = '';
    _.each(data, (function(_this) {
      return function(line) {
        var newEmbed, newLine;
        if (line.type === 'paragraph') {
          newLine = _this.appendLine(document.createElement(dom.DEFAULT_BLOCK_TAG));
          return newLine.node.innerHTML = line.data;
        } else {
          return newEmbed = _this.appendEmbed(document.createElement(dom.DEFAULT_EMBED_TAG), line.type, line.data);
        }
      };
    })(this));
    return this.rebuild();
...
},{"../lib/dom":17,"../lib/linked-list":18,"../lib/normalizer":19,"./embed":10,"./format":11,"./line":13,"lodash":1,"rich-text/lib/delta":3}],9:[function(_dereq_,module,exports){
var Document, Editor, Embed, Line, Selection, dom, _;
...
Editor = (function() {
  ...
  function Editor(root, quill, options) {
    ...
    this.doc = new Document(this.root, this, this.options);
        ...
        return function() {
          ...
          _.each(delta.ops, function(op) {
            if (typeof op.attributes !== 'undefined' && op.attributes.embed) {
              _this._insertEmbedAt(index, op.attributes.type, op.attributes.data);
              return index += 1;
  ...
  Editor.prototype._insertEmbedAt = function(index, type, data) {
    var offset, oldLine, _ref;
    _ref = this.doc.findLineAt(index), oldLine = _ref[0], offset = _ref[1];
    this.doc.insertEmbedBefore(document.createElement(dom.DEFAULT_EMBED_TAG), oldLine, type, data);
    if (oldLine.next) {
      return this.doc.removeLine(oldLine);
    }
  };

  Editor.prototype._deleteAt = function(index, length) {
    var firstLine, offset, _ref;
    if (length <= 0) {
      return;
    }
    _ref = this.doc.findLineAt(index), firstLine = _ref[0], offset = _ref[1];
    if (firstLine.node.className === Embed.CLASS_NAME) {
      if (firstLine.next.length <= 1) {
        this.doc.removeLine(firstLine.next);
      }
      this.selection.selectEmbed(firstLine);
      return;
    }
    return this.selection.shiftAfter(index, -1 * length, (function(_this) {
      return function() {
        var curLine, deleteLength, mergeFirstLine, nextLine, _ref1;
        _ref1 = _this.doc.findLineAt(index), firstLine = _ref1[0], offset = _ref1[1];
        curLine = firstLine;
...
},{"../lib/dom":17,"./document":8,"./embed":10,"./line":13,"./selection":14,"lodash":1}],10:[function(_dereq_,module,exports){
var Delta, Embed, Format, Leaf, Line, LinkedList, Normalizer, dom, _,
...
Embed = (function(_super) {
  __extends(Embed, _super);

  Embed.CLASS_NAME = 'ql-embed';

  Embed.ID_PREFIX = 'ql-embed-';

  function Embed(doc, node, type, data) {
    this.doc = doc;
    this.node = node;
    this._initController = __bind(this._initController, this);
    this._clickListener = __bind(this._clickListener, this);
    this.isEmbed = true;
    this._initController(type, data);
    this.node.setAttribute('contenteditable', false);
    this._clickListener();
    Embed.__super__.constructor.call(this, this.doc, this.node, Embed);
  }

  Embed.prototype._clickListener = function() {
    return dom(this.node).on('click', (function(_this) {
      return function() {
        return _this.doc.editor.selection.selectEmbed(_this);
      };
    })(this));
  };

  Embed.prototype._initController = function(type, data) {
    var controller, initialState;
    if (!_.isString(type)) {
      return;
    }
    controller = this.doc.editor.quill.getEmbed(type).controller;
    initialState = {
      type: type,
      data: data
    };
    return this.controller = controller(this.node, initialState);
  };

  Embed.prototype.getJSON = function() {
  ...
  Embed.prototype.optimize = function() {
    return this.rebuild();
  };

  Embed.prototype.rebuild = function(force) {
    if (force == null) {
      force = false;
    }
    this.resetContent();
  ...
  Embed.prototype.resetContent = function() {
    if (this.node.id !== this.id) {
      this.node.id = this.id;
    }
    this.outerHTML = this.node.outerHTML;
    this.length = 1;
    return this.delta = new Delta();
  };
...
},{"../lib/dom":17,"../lib/linked-list":18,"../lib/normalizer":19,"./format":11,"./leaf":12,"./line":13,"lodash":1,"rich-text/lib/delta":3}],11:[function(_dereq_,module,exports){
var Format, dom, _;
...
},{"../lib/dom":17,"lodash":1}],12:[function(_dereq_,module,exports){
...
},{"../lib/dom":17,"../lib/linked-list":18,"./format":11,"lodash":1}],13:[function(_dereq_,module,exports){
...
Line = (function(_super) {
  ...
  function Line(doc, node, base) {
    this.doc = doc;
    this.node = node;
    if (typeof base === 'undefined') {
      base = Line;
    }
    this.id = _.uniqueId(base.ID_PREFIX);
    this.formats = {};
    dom(this.node).removeClass('ql-embed');
    dom(this.node).addClass(base.CLASS_NAME);
    this.rebuild();
...
},{"../lib/dom":17,"../lib/linked-list":18,"../lib/normalizer":19,"./format":11,"./leaf":12,"./line":13,"lodash":1,"rich-text/lib/delta":3}],14:[function(_dereq_,module,exports){
var Embed, Leaf, Normalizer, Range, Selection, dom, _;
...
Selection = (function() {
  ...
  Selection.prototype.update = function(source) {
    var compare, emit, focus, range, toEmit;
    focus = this.checkFocus();
    range = this.getRange(true);
    compare = Range.compare(range, this.range);
    if (!compare) {
      this._removeSelectedEmbed();
    }
    emit = source !== 'silent' && ((!compare) || focus !== this.focus);
  ...
  Selection.prototype.selectEmbed = function(embed) {
    if (this.selectedEmbed) {
      dom(this.selectedEmbed.node).removeClass('selected');
    }
    this.selectedEmbed = embed;
    return dom(this.selectedEmbed.node).addClass('selected');
  };

  Selection.prototype._removeSelectedEmbed = function() {
    if (this.selectedEmbed) {
      dom(this.selectedEmbed.node).removeClass('selected');
      return this.selectedEmbed = false;
    }
  };
...
},{"../lib/dom":17,"../lib/normalizer":19,"../lib/range":21,"./embed":10,"./leaf":12,"lodash":1}],15:[function(_dereq_,module,exports){
...
},{"./modules/authorship":22,"./modules/image-tooltip":23,"./modules/keyboard":24,"./modules/link-tooltip":25,"./modules/multi-cursor":26,"./modules/paste-manager":27,"./modules/toolbar":28,"./modules/tooltip":29,"./modules/undo-manager":30,"./quill":31}],16:[function(_dereq_,module,exports){
...
},{"./dom":17,"./picker":20}],17:[function(_dereq_,module,exports){
...
},{"./dom":17,"./normalizer":19,"lodash":1}],21:[function(_dereq_,module,exports){
var Range, _;
...
},{"lodash":1}],22:[function(_dereq_,module,exports){
...
},{"../quill":31}],23:[function(_dereq_,module,exports){
...
},{"../quill":31,"./tooltip":29}],24:[function(_dereq_,module,exports){
...
Keyboard = (function() {
  ...
  Keyboard.prototype._initDeletes = function() {
    return this.addHotkey([dom.KEYS.DELETE, dom.KEYS.BACKSPACE], (function(_this) {
      return function(range, hotkey) {
        var start;
        if (_this.quill.editor.selection.selectedEmbed) {
          if (_this.quill.editor.selection.checkFocus()) {
            _this.quill.editor.doc.removeLine(_this.quill.editor.selection.selectedEmbed);
            return false;
          } else {
            return true;
          }
        } else if ((range != null) && _this.quill.getLength() > 1) {
  ...
  Keyboard.prototype._initEnter = function() {
    return this.addHotkey(dom.KEYS.ENTER, (function(_this) {
      return function(range, hotkey) {
        if (_this.quill.editor.selection.selectedEmbed) {
          if (_this.quill.editor.selection.checkFocus()) {
...
},{"../quill":31}],25:[function(_dereq_,module,exports){
...
},{"../quill":31,"./tooltip":29}],26:[function(_dereq_,module,exports){
...
},{"../quill":31,"eventemitter2":2}],27:[function(_dereq_,module,exports){
...
PasteManager = (function() {
  ...
  PasteManager.prototype._paste = function(e) {
    ...
    if (this.quill.editor.selection.selectedEmbed) {
    ...
    return _.defer((function(_this) {
      return function() {
        ...
        doc = new Document(_this.container, null, _this.quill.options);
        delta = doc.toDelta();
...
},{"../core/document":8,"../quill":31}],28:[function(_dereq_,module,exports){
...
},{"../lib/normalizer":19,"../quill":31}],30:[function(_dereq_,module,exports){
...
},{"../quill":31}],31:[function(_dereq_,module,exports){
...
Quill = (function(_super) {
  ...
  Quill.registerEmbed = function(name, controller) {
    if (Quill.embeds[name] != null) {
      console.warn("Overwriting " + name + " embed controller");
    }
    return Quill.embeds[name] = controller;
  };
  ...
  Quill.prototype.addEmbed = function(name, options) {
    var embedClass;
    embedClass = Quill.embeds[name];
    if (embedClass == null) {
      throw new Error("Cannot load " + name + " embed controller. Are you sure you registered it?");
    }
    this.embeds[name] = embedClass(options);
  ...
  Quill.prototype.getJSON = function() {
    return this.editor.doc.getJSON();
  };
  ...
  Quill.prototype.getEmbed = function(name) {
    return this.embeds[name];
  };

  Quill.prototype.getEmbeds = function() {
    return this.embeds;
  };
  ...
  Quill.prototype.insertEmbed = function(type, data, index, source) {
    var attributes, delta;
    attributes = {
      embed: true,
      type: type,
      data: data
  ...
  Quill.prototype.setJSON = function(data, source) {
    if (source == null) {
      source = Quill.sources.API;
    }
    this.editor.doc.buildFromJSON(data);
    return this.editor.checkUpdate(source);
  };
...
},{"../package.json":7,"./core/editor":9,"./core/format":11,"./lib/dom":17,"./lib/range":21,"./themes/base":33,"./themes/snow":34,"eventemitter2":2,"lodash":1,"rich-text/lib/delta":3}],32:[function(_dereq_,module,exports){
...
},{"../../lib/color-picker":16,"../../lib/dom":17,"../../lib/picker":20,"../base":33,"lodash":1}]},{},[15])(15)
});