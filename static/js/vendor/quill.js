...
Format = (function() {
  ...
  Format.FORMATS = {
    ...
    image: {
      ...
      tag: 'IMG',
      targets: ['src', 'data-temp-id', 'class']
    },
    align: {
      type: Format.types.LINE,
      style: 'textAlign',
  ...
  Format.prototype.add = function(node, value) {
    ...
    if (_.isString(this.config.template)) {
      return this.addShortcode(node, value);
    }
    ...
    if (_.isObject(this.config.targets)) {
      _.each(this.config.targets, function(attr) {
        return node.setAttribute(attr, value[attr]);
      });
    }
    if (_.isString(this.config.style) || _.isString(this.config.attribute) || _.isString(this.config["class"]) || _.isString(this.config.className)) {
      ...
      if (_.isString(this.config.className)) {
        dom(node).addClass(this.config.className);
      }
    }
    return node;
  };

  Format.prototype.addShortcode = function(node, value) {
    var imageNode, parentNode;
    if (!value) {
      return this.remove(node);
    }
    if (this.value(node) === value) {
    ...
    console.log(dom(imageNode).wrap(parentNode));
    node = imageNode;
    console.log(node);
    return node;
  };
